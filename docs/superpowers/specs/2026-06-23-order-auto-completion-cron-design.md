...
- `DeliveredAt *time.Time`: thời điểm đơn được chuyển sang `delivered`.
- `HasComplaint bool`: mặc định `false`; nếu là `true`, cron sẽ bỏ qua đơn hàng.

Thêm hằng trạng thái:

- `OrderStatusCompleted = "completed"`

Trong worktree hiện có thay đổi chưa commit ở `internal/models/order.go` với lỗi trùng tên `OrderStatusDelivered` cho cả `"delivered"` và `"completed"`. Khi triển khai phải sửa thành hằng riêng `OrderStatusCompleted`.
...
- Khi status mới là `delivered`, service set `delivered_at = now`.
- Trong phạm vi này, chỉ cron job tự động chuyển `delivered -> completed`.
- Đơn có `has_complaint = true` sẽ giữ ở trạng thái `delivered`.
- Nếu sau này `has_complaint` được đổi lại thành `false`, cron kế tiếp có thể hoàn tất đơn nếu `delivered_at` đã quá 7 ngày.
...
-> service cập nhật từng đơn trong transaction
